...
describe('getGasPrice', () => {
	...
	it('should call rpcMethods.getGasPrice with expected parameters', async () => {
		...
		expect(ethRpcMethods.getGasPrice).toHaveBeenCalledWith(web3Context.requestManager);
	});
	...
	it('should format mockRpcResponse using provided return format', async () => {
		...
		(ethRpcMethods.getGasPrice as jest.Mock).mockResolvedValueOnce(mockRpcResponse);
